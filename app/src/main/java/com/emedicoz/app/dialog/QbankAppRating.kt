...
class QbankAppRating: BottomSheetDialogFragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        imageView7.setOnClickListener(View.OnClickListener {
            dismiss()
            SharedPreference.getInstance().saveQbankTestStatus(Const.QBANK_TEST_STATUS, "")
        })

        val feedbackText = editFeedback.text
        rating = 0f
        //test_series_id = SharedPreference.getInstance().getTestSerieId(Const.QBANK_TEST_SERIES_ID, "")
        //q_bank = Constants.TestType.Q_BANK             //SharedPreference.getInstance().getString(Constants.Extras.TEST_TYPE)
...
//                } else {
//                    saveRatingAll(SharedPreference.getInstance().loggedInUser.id, test_series_id, rating, editFeedback.text.toString(), q_bank)
//                }
...
//                ratingRB.visibility = View.VISIBLE
                ...
                //saveRatingAll(SharedPreference.getInstance().loggedInUser.id, test_series_id, rating, "", q_bank)
