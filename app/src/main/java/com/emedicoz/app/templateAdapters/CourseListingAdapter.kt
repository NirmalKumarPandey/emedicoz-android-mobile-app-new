...
class CourseListingAdapter(private val trendingCourseList: ArrayList<Course>, private val context: Context,
                           private val isWishlist: Boolean, private val viewModel: CourseListingViewModel, private val fragType: String)
    : RecyclerView.Adapter<CourseListingAdapter.RecordCoursesListItemTrendingViewHolder>() {
                ...
                if (response.body() != null) {
                    ...
                    try {
                        ...
                        if (jsonResponse.optBoolean(Const.STATUS)) {
                            ...
                            if (GenericUtils.isListEmpty(trendingCourseList)) {
                                viewModel.publishResult.postValue(null)
                            } else
                                Toast.makeText(context, jsonResponse.optString(Constants.Extras.MESSAGE), Toast.LENGTH_SHORT).show()
                            notifyDataSetChanged()
                        } else {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecordCoursesListItemTrendingViewHolder {
